---
title: KUKA Loop 架構選項報告 v0（VISION BASE Lab 版）
...
> 本檔保留 VISION BASE Lab 自家網路設定與控制機 IP 等具體脈絡，是 lab 自家的 KUKA 控制方案紀錄。較通用、去專屬化的版本另存於 `04_知識庫/physical-ai/robotics/kuka/`。
...
- **銜接點**：Grasshopper 已能處理點雲 / 幾何，RGB-D → Rhino 是熟悉路徑
...
- **銜接點**：Grasshopper 已處理過影像 → 幾何，這層是它的「升級版」
...
- **銜接點**：KUKA.PRC 從 Grasshopper 出 path 是熟練操作。可組合：PRC 出粗 path → cuMotion 做避障 fine-tune
...
- 若 lab 是 **KR Agilus + KRC4/5** → 走 EKI-XML，上手快，Isaac 整合中等
- 若是 **iiwa** → FRI 真實時控制，是「Isaac 原生支援」最深的，但需要學 Sunrise/Java
...
| # | 場景 | 動作描述 | 技術難度 | 建築場景相關度 | Demo 殺傷力 | 預估時程 |
|---|---|---|---|---|---|---|
...
- 失敗模式: 累積誤差、堆疊穩定性
- 評語: 對 PRC 經驗最對胃口，視覺加分多，能展現「BIM → 實體」的 narrative
...
- 評語: 建築工地真實 use case。風險：要採缺陷資料 train
...
- 評語: 數位分身核心場景。但工程量大，v1 太重

**不下結論**：5 個場景各有特色。請使用者依下列 3 個維度選：

1. 想對誰 demo？
2. 是否能在 6-8 週內看到實體成果？
3. 場景 narrative 要打哪個方向？
...
- **Sim-first**：展示「我們已掌握 Isaac 棧」。但會被問「實際能動嗎」
- **Real-first**：實地展示。但 v1 學到的東西難複製到第二台機器
...
2. **5 個場景中，第一個 loop 要選哪個（或變形）？**
   決策因素：demo 殺傷力 vs 學習曲線 vs 6-8 週可達性。
...
6. **執行人力 全職還是 part-time？**
7. **Lab 空間：KUKA 在哪？電源、安全圍欄、網路？** — 影響 real-first 可行性
8. **是否要為 v1 預留 v2 scale path？**

---

## Lab 現場銜接點

- KUKA 控制 PC：`VB-KUKA-Only.local`（`192.168.0.82`）— 詳見 [[network-inventory]]
- 開發機：`DESKTOP-JDT5BJ8`（`192.168.0.220`，Windows）
- 同網段 NAS：`VISIONBASE_NAS.local`（`192.168.0.85`）
- 介面選型細節：見 [[kuka-control-feasibility]]
...
理由：
- 用上 KUKA.PRC + Grasshopper 既有技能
- 6-8 週可看到實體 demo
- Sim 階段學 Isaac，real 階段給殺傷力
- 不需採購工業級 LIDAR / 力控末端，預算可控

**注意**：這只是 sensible default，不是推薦。等前 5 題回答完再 lock。

---

## 附錄 B：已會 vs 要學

| 元件 | 現狀 | First loop 要補什麼 |
|---|---|---|
